...
interface AuthCtx {
  ...
  roles: AppRole[];
  profile: { id: string; nome: string; email: string; cargo?: string; department_id?: string; must_change_password?: boolean } | null;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function loadUserData(uid: string) {
    const [{ data: r }, { data: p }] = await Promise.all([
      ...
      supabase.from("profiles").select("id,nome,email,ativo,cargo,department_id,must_change_password").eq("id", uid).maybeSingle(),
    ]);
